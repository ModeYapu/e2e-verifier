...
async function main() {
  ...
  try {
    ...
    const rows = await page.$$('div[role="row"], [class*="row"]');
    console.log(`Row elements found: ${rows.length}`);
    const cells = await page.$$('[role="gridcell"], [class*="cell"]');
    console.log(`Cell elements found: ${cells.length}`);
    ...
    for (const btn of allButtons) {
      ...
      const allText = `${text || ''} ${html || ''} ${cls || ''} ${ariaLabel || ''} ${titleAttr || ''}`.toLowerCase();
