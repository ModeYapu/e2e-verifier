...
/**
 * Merge multiple scripts into one test suite (fallback without LLM)
 */
export function mergeScriptsFallback(
  scripts: string[],
  ...
  for (let i = 0; i < scripts.length; i++) {
    ...
    const extracted = extractMainLogic(script, i);
    testFunctions.push(extracted);
  }

  const authBlock = config.auth ? generateAuthBlock(config) : '';
 ...
 */
export function extractMainLogic(script: string, index: number): string {
 ...
 */
export function generateAuthBlock(config: ExploreConfig): string {
  if (!config.auth) return '';

  const auth = config.auth;
  const loginUrl = auth.loginUrl || config.url;
 ...
 */
export function generateHeuristicPlan(analyses: PageAnalysis[]): TestPlan {
 ...
 */
export function generateLoginPreamble(config: ExploreConfig): string {
  if (!config.auth) return '';
  const auth = config.auth;
  const loginUrl = auth.loginUrl || config.url;
 ...
 */
export function generateHeuristicScript(
  testCase: TestCase,
  ...
  const loginPreamble = generateLoginPreamble(config);
 ...
 */
export function buildSiteMap(pages: PageAnalysis[], rootUrl: string): SiteMapNode {
...
export async function performLogin(
  ...
  const loginUrl = auth.loginUrl || configUrl;
  ...
  if (auth.successUrlPattern) {
    await page.waitForURL(new RegExp(auth.successUrlPattern), { timeout: 10000 });
  } else {
    await page.waitForTimeout(3000);
  }
}
